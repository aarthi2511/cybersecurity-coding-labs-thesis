...
# IP pools - randomized per student
INTERNAL_IPS = ["192.168.1.10", "192.168.1.11"]

# Attacker IP pools (multiple options - random per student)
ATTACKER_IP_POOL = [
...
# Will be selected randomly based on seed
ATTACKER_IPS = []
SUSPICIOUS_IP = None
...
def generate_logs(filename="auth.log"):
    ...
    # Select random IPs for this student (based on their seed)
    ATTACKER_IPS = [
    ...
    with open(filename, "w") as f:
        for i in range(TOTAL_LOG_ENTRIES):
            # Base timestamp (predictable: 3 seconds between entries)
            ...
            if 200 <= i <= 245:
                ...
                ip = ATTACKER_IPS[0]
                status = "Failed"
                user = random.choice(USERS)  # Try multiple users
                ...
                # SCENARIO 3: Slow brute-force (scattered attempts, avoiding overlap)
                ip = ATTACKER_IPS[1]
                status = "Failed"
                ...
                ip = random.choice(ATTACKER_IPS[2:5])  # Rotate through 3 IPs
                status = "Failed"
                user = "admin"  # All attacking same user
